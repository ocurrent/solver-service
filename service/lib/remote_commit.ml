type t = { repo : string; hash : string } [@@deriving yojson]

let v ~repo ~hash = { repo; hash }
let repo x = x.repo
let hash x = x.hash
let pp f { repo; hash } = Fmt.pf f "%s (%s)" repo hash
let to_string checkout = to_yojson checkout |> Yojson.Safe.to_string
...
let of_yojson_or_fail json =
  match of_yojson json with Ok x -> x | Error ex -> failwith ex
