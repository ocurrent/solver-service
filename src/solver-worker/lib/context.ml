...
let ensure_dir path = if not (dir_exists path) then Unix.mkdir path 0o700
let get_tmp_dir t = t.state_dir / "tmp"
...
module Repo = struct
  type nonrec t = { context : t; url : Uri.t; lock : Lwt_mutex.t }
  ...
  let local_copy t = repos_dir t.context / id t

  let v context url =
    let url = Uri.of_string url in
    match Uri.scheme url with
    | Some "git" | Some "http" | Some "https" ->
        { context; url; lock = Lwt_mutex.create () }
  ...
  let has_commits t cs =
    ...
    if dir_exists local_repo then
      let rec aux = function
        ...
        | c :: cs -> (
            Lwt_process.exec ~cwd:local_repo
              ("", [| "git"; "cat-file"; "-e"; Hash.to_hex c |])
            >>= function
  ...
  (* Don't let ocaml-git try to init a new repository! *)
  ...
  let fetch ~switch ~log t =
    ...
    Process.check_call ~label:"git-reset" ~switch ~log
      [ "git"; "-C"; local_repo; "reset"; "--hard" ]
    >>!= fun () ->
    Process.check_call ~label:"git-submodule-sync" ~switch ~log
      [ "git"; "-C"; local_repo; "submodule"; "sync" ]
    >>!= fun () ->
    Process.check_call ~label:"git-submodule-deinit" ~switch ~log
      [ "git"; "-C"; local_repo; "submodule"; "deinit"; "--all"; "-f" ]
    >>!= fun () ->
...
let rec lwt_result_list_iter_s f = function
  | [] -> Lwt_result.return ()
  | x :: xs -> f x >>!= fun () -> lwt_result_list_iter_s f xs
...
let build_context t ~log ~tmpdir descr =
  match
    Cluster_api.Raw.Reader.JobDescr.commits_get_list descr
    |> List.map Hash.of_hex
  with
  | [] -> Lwt_result.return ()
  | c :: cs as commits ->
      let repository =
        repo t (Cluster_api.Raw.Reader.JobDescr.repository_get descr)
      in
      Lwt_mutex.with_lock repository.lock (fun () ->
          Lwt_switch.with_switch @@ fun switch ->
          (* Don't let the user cancel these operations. *)
          (Repo.has_commits repository commits >>!= function
           | true ->
               Log_data.info log "All commits already cached";
               Lwt_result.return ()
           | false -> Repo.fetch ~switch ~log repository)
          >>!= fun () ->
          let clone = Repo.local_copy repository in
          Process.check_call ~label:"git-submodule-update" ~switch ~log
            [ "git"; "-C"; clone; "submodule"; "update" ]
          >>!= fun () ->
          Process.check_call ~label:"git-reset" ~switch ~log
            [ "git"; "-C"; clone; "reset"; "--hard"; Hash.to_hex c ]
          >>!= fun () ->
          Process.check_call ~label:"git-submodule-sync" ~switch ~log
            [ "git"; "-C"; clone; "submodule"; "sync" ]
          >>!= fun () ->
          Process.check_call ~label:"git-submodule-deinit" ~switch ~log
            [ "git"; "-C"; clone; "submodule"; "deinit"; "--all"; "-f" ]
          >>!= fun () ->
          Process.check_call ~label:"git-clean" ~switch ~log
            [ "git"; "-C"; clone; "clean"; "-fdx" ]
          >>!= fun () ->
          let merge c =
            Process.check_call ~label:"git-merge" ~switch ~log
              ~env:git_merge_env
              [ "git"; "-C"; clone; "merge"; Hash.to_hex c ]
          in
          ...
          if include_git descr then
            let cmd, is_success =
              if Sys.win32 then
                ...
                ([ "cp"; "-a"; clone / ".git"; tmpdir / ".git" ], fun s -> s = 0)
            in
            Process.check_call ~label:"cp .git" ~switch ~log ~is_success cmd
            >>!= fun () ->
            Process.check_call ~label:"git-config" ~switch ~log
              [
                "git";
...
let with_build_context t ~log descr fn =
  ...
  build_context t ~log ~tmpdir descr >>!= fun () -> fn tmpdir
